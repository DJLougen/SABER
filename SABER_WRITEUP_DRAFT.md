...
SABER is my version of abliteration: a constrained, multi-objective refusal-editing method that tries to suppress refusal behavior while limiting behavioral drift.
...
> Select many candidate refusal directions, rank them by how well they separate refusal behavior, estimate how entangled they are with ordinary capability behavior, and ablate them with different strengths depending on that tradeoff.

## One-Sentence Method

SABER combines separability-ranked multi-direction refusal subspaces with capability-entanglement scoring, then applies differential ablation strengths to reduce refusals while limiting behavioral drift.
...
5. Pareto evaluation over refusal rate and behavioral drift, measured here with keyword refusal and KLD from the base model.
...
The current target model is `google/gemma-4-E4B-it`.
