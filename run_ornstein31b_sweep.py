#!/usr/bin/env python3
"""Sweep SABER configs on Ornstein-31B-it (Gemma 4 31B)."""
...
# Based on Ornstein-27B (gk=25) and Gemma-4-E2B (gk=10):
# 31B Gemma 4 should need gk ~15-25
...
# Summary
print('\n' + '='*80)
print(f'Ornstein-31B Sweep: baseline refusal={baseline_refusal:.0%}, PPL={baseline_ppl:.1f}')
print('-'*80)
...
if zero_refusal:
    ...
    result = saber.run(harmful_prompts=harmful, harmless_prompts=harmless, capability_prompts=capability)
    save_dir = Path('/tmp/saber/saber_output_ornstein31b')
    save_dir.mkdir(parents=True, exist_ok=True)
    model.save_pretrained(save_dir, safe_serialization=True)
    tokenizer.save_pretrained(save_dir)
    logger.info(f'Best model saved to {save_dir}')

with open('/tmp/saber/saber_ornstein31b_sweep_results.json', 'w') as f:
